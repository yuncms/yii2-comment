...
<div class="media">
    <div class="media-left">
        <a href="<?=Url::to(['/user/profile/show','id'=>$model->user_id])?>" target="_blank">
            <img class="media-object avatar-27" alt="<?=Html::encode($model->user->username)?>" src="<?=$model->user->getAvatar()?>" >
        </a>
    </div>
    <div class="media-body">
        <div class="media-heading">
            <a href="<?=Url::to(['/user/profile/show','id'=>$model->user_id])?>" target="_blank"><?=$model->user->username?></a>
            <?php if($model->to_user_id):?>
                <span class="text-muted">回复 </span>
                <a href="<?=Url::to(['/user/profile/show','id'=>$model->to_user_id])?>" target="_blank"><?=$model->toUser->username?></a>
            <?php endif;?>
        </div>
        ...
        <div class="media-footer">
            ...
            <?php if(!Yii::$app->user->isGuest && $model->user_id != Yii::$app->user->id):?>
                <a href="#" class="ml-10 comment-reply"
                   ...
                   data-source_type="<?=$model->source_type?>" data-message="回复 <?=Html::encode($model->user->username)?>"><i class="fa fa-reply"></i> 回复</a>
            <?php endif;?>
        </div>
